...
def get_2d_array(im_path, shape):
    ...
    >>> get_2d_array("64x64.png", shape=(32,32,1)).shape
    """
    im_color = cv2.imread(im_path)  # Read the image as a numpy array.
    # Shape = (64,64,3) (x_pixels,y_pixels, color_channels)
    im_color = cv2.resize(im_color, (0, 0), fx=0.5, fy=0.5)  # Rescale the image
    # Shape = (32,32,3) (x_pixels*scale, y_pixels*scale, color_channels)
    im = np.zeros(shape=shape)  # Create an empty array with the final shape (32,32,1)
...
def get_label(name):
    ...
    9
    """
    return int(int(name.split("_")[1]) - 1)
...
def main():
    ...
    for i, file in enumerate(os.listdir(path)):
        ...
        image = get_2d_array(os.path.join(path, file), shape=(32, 32, 1))
        images[i] = image
        labels[i, label] = 1
    ...
    # Confirm arrays have correct shape
